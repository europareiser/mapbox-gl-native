...
canonicalizeTileURL(const std::string& str, const SourceType type, const uint16_t tileSize) {
    const char* mapbox = ".mapbox.com";
    const size_t mapboxLen = strlen(mapbox);
    ...
    // Has to be *.mapbox.com/v4/, with a valid filename + extension
    if (url.domain.second < mapboxLen ||
        str.compare(url.domain.first + url.domain.second - mapboxLen, mapboxLen, mapbox) != 0 ||
        str.compare(url.path.first, versionLen, version) != 0 || path.filename.second == 0 ||
